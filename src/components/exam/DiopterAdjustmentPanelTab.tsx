...
export function DiopterAdjustmentPanelTab({
  ...
  React.useEffect(() => {
    ...
    const rightParsed =
      typeof rightRaw === "number" ? rightRaw : Number(rightRaw);
  ...
  const handleAdjust = (
    eye: "right_diopter" | "left_diopter",
    delta: number,
  ) => {
    if (!isEditing) return;
    ...
    console.log(
      `DiopterAdjustmentPanel: ${eye} change from ${current} to ${next}`,
    );
  ...
  const handleButtonHold = (
    field: "right_diopter" | "left_diopter",
    delta: number,
  ) => {
  ...
  const renderEye = (side: "right" | "left") => {
    ...
    let angle =
      ((-clampedDeviation + maxDeviation) / (2 * maxDeviation)) * Math.PI;
    ...
    return (
      <div
        style={{ direction: "rtl", padding: 24, borderRadius: 8 }}
        className="bg-accent/50 shadow-md"
      >
        ...
        >
          <svg width={110} height={110} style={{ display: "block" }}>
            ...
            {(() => {
              ...
              const leftEndXAbove =
                leftStartX + (centerX - leftStartX) * angleFraction;
              const leftEndYAbove =
                leftStartYAbove + (centerY - leftStartYAbove) * angleFraction;
              // Left side below
              const leftStartYBelow = y + lineHeight + lighterOffset;
              const leftEndXBelow =
                leftStartX + (centerX - leftStartX) * angleFraction;
              const leftEndYBelow =
                leftStartYBelow + (centerY - leftStartYBelow) * angleFraction;
              // Right side above
              const rightStartX = rightX + lineWidth;
              const rightStartYAbove = y - lighterOffset;
              const rightEndXAbove =
                rightStartX + (centerX - rightStartX) * angleFraction;
              const rightEndYAbove =
                rightStartYAbove + (centerY - rightStartYAbove) * angleFraction;
              // Right side below
              const rightStartYBelow = y + lineHeight + lighterOffset;
              const rightEndXBelow =
                rightStartX + (centerX - rightStartX) * angleFraction;
              const rightEndYBelow =
                rightStartYBelow + (centerY - rightStartYBelow) * angleFraction;
              return (
                <>
                  <rect
                    x={leftX}
            ...
            <g>
              <circle
                ...
                style={{
                  transition:
                    "cx 0.25s cubic-bezier(0.4,0,0.2,1), cy 0.25s cubic-bezier(0.4,0,0.2,1)",
                }}
  ...
  return (
    <Card className="examcard w-full pt-3 pb-4" style={{ direction: "rtl" }}>
      <CardContent
        className="flex w-full flex-row justify-center gap-4 px-4"
        style={{ scrollbarWidth: "none", margin: "0 auto" }}
      >
        {renderEye("left")}
        {renderEye("right")}
      </CardContent>
