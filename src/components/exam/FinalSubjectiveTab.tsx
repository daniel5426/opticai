...
function FinalSubjectiveSection({ eye, data, onChange, isEditing }: EyeSectionProps) {
  ...
  return (
    <div className="flex items-center gap-1 h-6 mb-3" dir="rtl">
      <div className="grid grid-cols-22 gap-4 flex-1 pb-2" dir="ltr">
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-final-sph`} className="text-[12px] block text-center">SPH</Label>}
          <Input id={`${eye}-final-sph`} type="number" step="0.25" value={getFieldValue("sph")} onChange={(e) => handleChange("sph", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.00" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-final-cyl`} className="text-[12px] block text-center">CYL</Label>}
          <Input id={`${eye}-final-cyl`} type="number" step="0.25" value={getFieldValue("cyl")} onChange={(e) => handleChange("cyl", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.00" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-final-ax`} className="text-[12px] block text-center">AXIS</Label>}
          <Input id={`${eye}-final-ax`} type="number" min="0" max="180" value={getFieldValue("ax")} onChange={(e) => handleChange("ax", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-final-pr-h`} className="text-[12px] block text-center">PR.H</Label>}
          <Input id={`${eye}-final-pr-h`} type="number" step="0.5" value={getFieldValue("pr_h")} onChange={(e) => handleChange("pr_h", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.0" />
        </div>
        <div className="col-span-2">
        ...
        <div className="col-span-2">
          ...
          <Input id={`${eye}-final-pr-v`} type="number" step="0.5" value={getFieldValue("pr_v")} onChange={(e) => handleChange("pr_v", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.0" />
        </div>
        <div className="col-span-2">
        ...
        <div className="col-span-2">
          ...
          <div className="relative" dir="ltr">
            <Input id={`${eye}-final-va`} type="number" step="0.1" value={getFieldValue("va")} onChange={(e) => handleChange("va", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1 pl-6" placeholder="0.0" />
        ...
        <div className="col-span-2">
          ...
          <Input id={`${eye}-final-j`} type="number" value={getFieldValue("j")} onChange={(e) => handleChange("j", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-final-pd-close`} className="text-[12px] block text-center">PD CLOSE</Label>}
          <Input id={`${eye}-final-pd-close`} type="number" step="0.5" value={getFieldValue("pd_close")} onChange={(e) => handleChange("pd_close", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-final-pd-far`} className="text-[12px] block text-center">PD FAR</Label>}
          <Input id={`${eye}-final-pd-far`} type="number" step="0.5" value={getFieldValue("pd_far")} onChange={(e) => handleChange("pd_far", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD" />
        </div>
      </div>
...
function CombinedFinalSubjFields({ finalSubjectiveData, onChange, isEditing, onVHConfirm }: { 
  ...
  return (
    <div className="flex items-center gap-1 h-10 mb-3" dir="rtl">
      <div className="grid grid-cols-22 gap-4 flex-1" dir="ltr">
        ...
        <div className="col-span-2">
        <div className="relative" dir="ltr">
            <Input id={`comb-final-va`} type="number" step="0.1" value={finalSubjectiveData.comb_va?.toString() || ""} onChange={(e) => onChange("comb_va", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1 pl-6" placeholder="0.0" />
            <span className="absolute left-2 top-[53%] transform -translate-y-1/2 text-[14px] text-gray-500 pointer-events-none">6/</span>
          </div>
        </div>
        <div className="col-span-2"></div>
        <div className="col-span-2">
          <Input type="number" step="0.5" value={finalSubjectiveData.comb_pd_close?.toString() || ""} onChange={(e) => onChange("comb_pd_close", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD Close" />
        </div>
        <div className="col-span-2">
          <Input type="number" step="0.5" value={finalSubjectiveData.comb_pd_far?.toString() || ""} onChange={(e) => onChange("comb_pd_far", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD Far" />
        </div>
      </div>
  ...
  return (
    <Card className="shadow-md border-none">
      <CardContent className="px-4 pt-4 space-y-1">
        ...
        <FinalSubjectiveSection eye="R" data={finalSubjectiveData} onChange={onFinalSubjectiveChange} isEditing={isEditing} />
        <CombinedFinalSubjFields finalSubjectiveData={finalSubjectiveData} onChange={onFinalSubjectiveChange} isEditing={isEditing} onVHConfirm={onVHConfirm} />
        <FinalSubjectiveSection eye="L" data={finalSubjectiveData} onChange={onFinalSubjectiveChange} isEditing={isEditing} />
      </CardContent>
    </Card>
  );
} 