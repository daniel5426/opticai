...
export function ExamPreviewModal({
  ...
  const resolvedActiveOldRefractionTabs = useMemo(() => {
    return Object.fromEntries(
      Object.entries(oldRefractionTabs).map(([cardId, tabs]) => [
        ...
        tabs.includes(activeOldRefractionTabs[cardId])
          ? activeOldRefractionTabs[cardId]
          : tabs[0],
      ]),
    );
  ...
  return (
    <CustomModal
      ...
      subtitle={
        examData
          ? `${new Date(examData.exam_date).toLocaleDateString("he-IL")}${examData.dominant_eye ? ` · עין דומיננטית: ${examData.dominant_eye === "R" ? "ימין" : "שמאל"}` : ""}`
          : undefined
      }
