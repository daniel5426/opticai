    ...
    <div className="flex items-center gap-1 h-6 mb-3" dir="rtl">
      <div className="grid grid-cols-20 gap-4 flex-1 pb-2" dir="ltr">
        <div className="col-span-2">
          ...
          <Input id={`${eye}-subj-fa`} type="number" step="0.1" value={getFieldValue("fa")} onChange={(e) => handleChange("fa", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="FA" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-fa-tuning`} className="text-[12px] block text-center">FA TUN</Label>}
          <Input id={`${eye}-subj-fa-tuning`} type="number" step="0.1" value={getFieldValue("fa_tuning")} onChange={(e) => handleChange("fa_tuning", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="FA TUN" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-sph`} className="text-[12px] block text-center">SPH</Label>}
          <Input id={`${eye}-subj-sph`} type="number" step="0.25" value={getFieldValue("sph")} onChange={(e) => handleChange("sph", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.00" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-cyl`} className="text-[12px] block text-center">CYL</Label>}
          <Input id={`${eye}-subj-cyl`} type="number" step="0.25" value={getFieldValue("cyl")} onChange={(e) => handleChange("cyl", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.00" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-ax`} className="text-[12px] block text-center">AXIS</Label>}
          <Input id={`${eye}-subj-ax`} type="number" min="0" max="180" value={getFieldValue("ax")} onChange={(e) => handleChange("ax", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-pris`} className="text-[12px] block text-center">PRIS</Label>}
          <Input id={`${eye}-subj-pris`} type="number" step="0.5" value={getFieldValue("pris")} onChange={(e) => handleChange("pris", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.0" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-base`} className="text-[12px] block text-center">BASE</Label>}
          <Input id={`${eye}-subj-base`} type="number" step="0.1" value={getFieldValue("base")} onChange={(e) => handleChange("base", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="0.0" />
        </div>
        <div className="col-span-2">
        {eye === "R" && <Label htmlFor={`${eye}-old-va`} className="text-[12px] block text-center">VA</Label>}
          <div className="relative" dir="ltr">
            <Input id={`${eye}-old-va`} type="number" step="0.1" value={getFieldValue("va")} onChange={(e) => handleChange("va", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1 pl-6" placeholder="0.0" />
        ...
        <div className="col-span-2">
          ...
          <Input id={`${eye}-subj-pd-close`} type="number" step="0.5" value={getFieldValue("pd_close")} onChange={(e) => handleChange("pd_close", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD" />
        </div>
        <div className="col-span-2">
          {eye === "R" && <Label htmlFor={`${eye}-subj-pd-far`} className="text-[12px] block text-center">PD FAR</Label>}
          <Input id={`${eye}-subj-pd-far`} type="number" step="0.5" value={getFieldValue("pd_far")} onChange={(e) => handleChange("pd_far", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD" />
        </div>
      </div>
...
function CombinedSubjFields({ subjectiveData, onSubjectiveChange, isEditing, onVHConfirm, onMultifocalClick }: { 
  ...
  return (
    <div className="flex items-center gap-1 h-10 mb-3" dir="rtl">
      <div className="grid grid-cols-20 gap-4 flex-1" dir="ltr">
        <div className="col-span-2">
          <Input type="number" step="0.1" value={subjectiveData.comb_fa?.toString() || ""} onChange={(e) => onSubjectiveChange("comb_fa", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="FA" />
        </div>
        <div className="col-span-2">
          <Input type="number" step="0.1" value={subjectiveData.comb_fa_tuning?.toString() || ""} onChange={(e) => onSubjectiveChange("comb_fa_tuning", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="FA TUN" />
        </div>
        ...
        <div className="col-span-2">
        <div className="relative" dir="ltr">
            <Input id={`comb-subj-va`} type="number" step="0.1" value={subjectiveData.comb_va?.toString() || ""} onChange={(e) => onSubjectiveChange("comb_va", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1 pl-6" placeholder="0.0" />
            <span className="absolute left-2 top-[53%] transform -translate-y-1/2 text-[14px] text-gray-500 pointer-events-none">6/</span>
          </div>
        </div>
        <div className="col-span-2">
          <Input type="number" step="0.5" value={subjectiveData.comb_pd_close?.toString() || ""} onChange={(e) => onSubjectiveChange("comb_pd_close", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD Close" />
        </div>
        <div className="col-span-2">
          <Input type="number" step="0.5" value={subjectiveData.comb_pd_far?.toString() || ""} onChange={(e) => onSubjectiveChange("comb_pd_far", e.target.value)} disabled={!isEditing} className="h-8 text-xs px-1" placeholder="PD Far" />
        </div>
      </div>
  ...
  return (
    <Card className="shadow-md border-none">
      <CardContent className="px-4 pt-4 space-y-1">
        ...
        <SubjectiveSection eye="R" data={subjectiveData} onChange={onSubjectiveChange} isEditing={isEditing} />
        <CombinedSubjFields subjectiveData={subjectiveData} onSubjectiveChange={onSubjectiveChange} isEditing={isEditing} onVHConfirm={onVHConfirm} onMultifocalClick={onMultifocalClick} />
        <SubjectiveSection eye="L" data={subjectiveData} onChange={onSubjectiveChange} isEditing={isEditing} />
      </CardContent>
    </Card>
  );
} 