...
interface InputProps extends React.ComponentProps<"input"> {
  ...
  /** Layout classes for the outer control wrapper (for example margins). */
...
function formatSignedNumber(
  ...
  const formatted =
    finalPrecision > 0 ? num.toFixed(finalPrecision) : num.toString();
...
function isStepAligned(
  value: number,
  stepProp: string | number | undefined,
  minProp: string | number | undefined,
): boolean {
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const isNumeric =
      isNumericInput || inputMode === "decimal" || inputMode === "numeric";
    ...
    const handleStep = React.useCallback(
      (increment: boolean) => {
        ...
        if (isNumericInput) {
          ...
          const formatted = formatSignedNumber(
            newVal,
            !!showPlus,
            stepPrecision,
          );
    ...
    const handleDecimalSeparatorDelete = (
      input: HTMLInputElement,
      key: string,
    ) => {
    ...
    const maxValue =
      minValue !== undefined &&
      rawMaxValue !== undefined &&
      !isNaN(minValue) &&
      !isNaN(rawMaxValue) &&
      rawMaxValue < minValue
        ? undefined
        : rawMaxValue;
    const isOutOfRange =
      isNumericInput &&
    ...
    const isStepMismatch =
      isNumericInput &&
      !isNaN(numericValue) &&
      localValue !== "" &&
      !isStepAligned(numericValue, props.step, props.min);
    const ariaInvalid =
      props["aria-invalid"] ?? (isOutOfRange || isStepMismatch);
    ...
    return (
      <div
        ...
        className={cn(
          ...
          prefix || suffix || showButtons || hasLeadingOverlay
            ? "relative"
            : "",
          showButtons && "overflow-hidden rounded-md",
          wrapperClassName,
        )}
      >
        {hasLeadingOverlay && (
          <div
            className={cn(
              "border-input bg-card absolute top-px bottom-px left-px z-20 overflow-hidden rounded-l-md border-r",
        ...
        <input
          ...
          className={cn(
            ...
            type === "number" || center
              ...
              : effectiveDir === "rtl"
                ? "text-right"
                : "text-left",
            `file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground disabled:cursor-default disabled:opacity-100 ${noBorder ? "border-none" : "border-input border"} flex h-9 w-full min-w-0 rounded-md py-1 text-base transition-[color] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium md:text-sm`,
            "focus-visible:border-ring ring-0 outline-none focus-visible:outline-none",
            "aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
            "[appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none",
            hasLeadingOverlay ? "" : prefix ? "pl-5.5" : "px-1",
            suffix ? "pr-5.5" : showButtons ? "pr-4" : "",
            className,
          )}
          {...(() => {
          ...
          onInput={(e) => {
            ...
            if (isNumericInput) {
              ...
              if (
                normalized.shouldAdvance &&
                !suppressClinicalAutoAdvanceRef.current
              ) {
          ...
          onBlur={(e) => {
            ...
            if (val !== "" && isNumericInput) {
              ...
              if (!isNaN(num)) {
                const finalValue = formatSignedNumber(
                  num,
                  !!showPlus,
                  stepPrecision,
                );
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className="text-destructive absolute top-0.5 left-0.5 z-30 flex h-3 w-3 items-center justify-center opacity-90"
              >
        ...
        {showButtons && (
          <div className="border-input bg-card absolute top-px right-px bottom-px z-20 flex w-3 flex-col overflow-hidden rounded-r-md border-l opacity-0 transition-opacity group-hover:opacity-100">
            <button
              type="button"
              tabIndex={-1}
              onMouseDown={(e) => {
              ...
              className="hover:bg-accent border-input text-muted-foreground hover:text-foreground active:bg-accent-foreground/10 flex flex-1 items-center justify-center border-b"
            >
              <ChevronUp size={10} />
            </button>
            <button
              ...
              onMouseUp={stopStep}
              onMouseLeave={stopStep}
              className="hover:bg-accent text-muted-foreground hover:text-foreground active:bg-accent-foreground/10 flex flex-1 items-center justify-center"
            >
              <ChevronDown size={10} />
            </button>
          </div>
