...
export function ReferralTable({
  ...
  const sortColumns = React.useMemo<SortColumns<Referral>>(
    () => ({
      ...
      urgency: {
        getValue: (referral) => referral.urgency_level,
        type: "rank",
  ...
  const handleRowClick = (referral: Referral) => {
    ...
    navigate({
      ...
      params: {
        clientId: String(referral.client_id),
  ...
  return (
    <div className="space-y-2.5" style={{ scrollbarWidth: "none" }}>
      <TableFiltersBar
        ...
        hasActiveFilters={
          Boolean(searchValue.trim()) || urgencyFilter !== ALL_FILTER_VALUE || referralTypeFilter !== ALL_FILTER_VALUE
        }
        onReset={() => {
          ...
          <TableBody>
            {loading ? (
              ...
              displayData.map((referral) => {
                ...
                return (
                  <TableRow key={referral.id} className="cursor-pointer" onClick={() => handleRowClick(referral)}>
                    <TableCell className="text-right">
                      {referral.date ? new Date(referral.date).toLocaleDateString("he-IL") : "-"}
                    </TableCell>
                    <TableCell className="text-right">{referral.type || "-"}</TableCell>
                    {clientId === 0 && (
                      <TableCell className="text-right">
                        <button
                          type="button"
                    ...
                    <TableCell className="text-right">{referral.recipient || "-"}</TableCell>
                    <TableCell>
