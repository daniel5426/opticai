...
export default function ReferralDetailPage() {
  ...
  const referralId = isNewReferral
    ? undefined
    : ((routeParams as any).referralId as string | undefined);
  ...
  const [compactPrescriptionFormData, setCompactPrescriptionFormData] =
    useState<CompactPrescriptionExam>({
      referral_id: 0,
    });
  ...
  const fieldHandlers = {
    [type]: (field: string, value: string) =>
      handleCompactPrescriptionChange(
        field as keyof CompactPrescriptionExam,
        value,
      ),
  };
  ...
  const handlePaste = () => {
    ...
    const copiedData = ExamFieldMapper.copyData(
      sourceData as any,
      compactPrescriptionFormDataRef.current as any,
      sourceType,
      type,
    );
    ...
    Object.entries(copiedData).forEach(([key, value]) => {
      if (key !== "id" && value !== undefined) {
        handleCompactPrescriptionChange(
          key as keyof CompactPrescriptionExam,
          String(value),
        );
      }
    });
      ...
      try {
        ...
        if (isNewReferral) {
          if (clientIdFromRoute) {
            setFormData((prev: Referral) => ({
              ...prev,
              client_id: Number(clientIdFromRoute),
            }));
          }
          setLoading(false);
          return;
        ...
        if (referral) {
          ...
          if (cp) {
            ...
            setCompactPrescriptionFormData((prev: CompactPrescriptionExam) => ({
              ...prev,
          ...
          setFormData((prev) => ({
            ...
            referral_data: {
              ...
              "clinical-findings": {
                r_iop:
                  (prev.referral_data as any)?.["clinical-findings"]?.r_iop ||
                  "",
                l_iop:
                  (prev.referral_data as any)?.["clinical-findings"]?.l_iop ||
                  "",
                clinical_impression:
  ...
  const handleCompactPrescriptionChange = (
    field: keyof CompactPrescriptionExam,
    value: any,
  ) => {
    ...
    return (
      <>
        <SiteHeader
          ...
          backLink="/clients"
          clientBackLink={
            clientIdFromRoute || formData.client_id
              ? `/clients/${clientIdFromRoute || formData.client_id}`
              : "/clients"
          }
          examInfo="הפניה"
          tabs={{
        ...
        <ClientSpaceLayout>
          ...
          >
            ...
            <div className="mb-8">
              <div className="bg-muted/40 w-full rounded-md p-4">
        ...
        >
          <div className="space-y-6">
            ...
            <Card className="w-full p-4 pt-3 shadow-md">
              <div
                className="grid w-full grid-cols-4 gap-x-3 gap-y-2"
                dir="rtl"
              >
                <div className="col-span-1">
                  <label className="text-base font-semibold">תאריך</label>
                ...
                <div className="col-span-1">
                  <label className="text-base font-semibold">רמת דחיפות</label>
                  <div className="h-1"></div>
                  <Select
                ...
                <div className="col-span-1">
                  <label className="text-base font-semibold">נמען</label>
                  <div className="h-1"></div>
                  <Input
                    name="recipient"
            ...
            <div className="relative grid grid-cols-1 items-stretch gap-6 lg:grid-cols-9">
              <div className="flex flex-col gap-6 lg:col-span-6">
                <Card className="w-full gap-4 p-4 pt-3 shadow-md">
                  ...
                  <div
                    className="grid w-full grid-cols-9 gap-x-4 gap-y-2"
                    dir="rtl"
                  >
                    <div className="col-span-7">
                      <label className="text-muted-foreground justify-center text-sm font-medium">
                        הערכה קלינית / אבחנה משוערת
                      </label>
                      <div className="h-1"></div>
                      <Input
                        value={
                          (formData.referral_data as any)?.["clinical-findings"]
                            ?.clinical_impression || ""
                        }
                        onChange={(e) =>
                          handleClinicalFindingsChange(
                            "clinical_impression",
                            e.target.value,
                          )
                        }
                    ...
                    <div className="col-span-1">
                      <label className="text-muted-foreground block w-full text-center text-sm font-medium">
                        R-IOP
                      </label>
                      <div className="h-1"></div>
                      <Input
                        ...
                        onChange={(e) =>
                          handleClinicalFindingsChange("r_iop", e.target.value)
                        }
                        suffix="mmHg"
                        disabled={!isEditing}
                        className={`h-9 text-sm ${isEditing ? "bg-white" : "bg-accent/50"} disabled:cursor-default disabled:opacity-100`}
                    ...
                    <div className="col-span-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleClinicalFindingsChange("l_iop", e.target.value)
                        }
                        suffix="mmHg"
                        disabled={!isEditing}
                        className={`h-9 text-sm ${isEditing ? "bg-white" : "bg-accent/50"} disabled:cursor-default disabled:opacity-100`}
              ...
              <div className="lg:col-span-3">
                <NotesCard
                  ...
                  onChange={(value) =>
                    setFormData((prev) => ({ ...prev, referral_notes: value }))
                  }
                  disabled={!isEditing}
