...
import { OpticalExam, OldRefractionExam, ObjectiveExam, SubjectiveExam, AdditionExam, Client, User, FinalSubjectiveExam, ExamLayout, ExamLayoutInstance, RetinoscopExam } from "@/lib/db/schema"
import { getRetinoscopExamByLayoutInstanceId, createRetinoscopExam, updateRetinoscopExam } from "@/lib/db/retinoscop-db"
...
export default function ExamDetailPage({
  ...
  const [retinoscopExam, setRetinoscopExam] = useState<RetinoscopExam | null>(null)
  ...
  const [retinoscopFormData, setRetinoscopFormData] = useState<RetinoscopExam>({} as RetinoscopExam)
      ...
      try {
        ...
        if (examId && !isNewMode) {
            ...
            if (layoutInstances.length > 0) {
              ...
              if (activeInstance) {
                ...
                if (activeLayout && activeLayout.layout_data && activeInstance.id) {
                  ...
                  const retinoscopData = await getRetinoscopExamByLayoutInstanceId(activeInstance.id)
                  ...
                  setRetinoscopExam(retinoscopData || null)
                }
              } else {
          ...
          if (defaultLayout) {
            ...
            setRetinoscopFormData({ layout_instance_id: defaultLayout.id || 0 } as RetinoscopExam)
          }
        }
  ...
  useEffect(() => {
    ...
    if (retinoscopExam) {
      setRetinoscopFormData({ ...retinoscopExam })
    } else if (!isNewMode) {
      const activeLayout = layoutTabs.find(tab => tab.isActive);
      if (activeLayout) {
        setRetinoscopFormData({ layout_instance_id: activeLayout.id } as RetinoscopExam);
      }
    }
  }, [exam, oldRefractionExam, objectiveExam, subjectiveExam, additionExam, finalSubjectiveExam, retinoscopExam, layoutTabs, isNewMode])
  ...
  const handleRetinoscopFieldChange = (field: keyof RetinoscopExam, rawValue: string) => {
    let processedValue: string | number | undefined = rawValue;

    const numericFields: (keyof RetinoscopExam)[] = [
      "r_sph", "r_cyl", "r_ax", "l_sph", "l_cyl", "l_ax"
    ];
    ...
    setRetinoscopFormData(prev => ({ ...prev, [field]: processedValue }));
  };
    ...
    if (formRef.current) {
      if (isNewMode) {
        ...
        if (newExam && newExam.id) {
          ...
          // Save retinoscop exam data if there's data
          let newRetinoscopExam: RetinoscopExam | null = null
          const hasRetinoscopData = Object.values(retinoscopFormData).some(value =>
            value !== undefined && value !== null && value !== ''
          );

          if (hasRetinoscopData) {
            newRetinoscopExam = await createRetinoscopExam({
              ...retinoscopFormData,
              layout_instance_id: layoutInstance.id!
            });
          }
        ...
        // Save retinoscop exam data if there's data
        let updatedRetinoscopExam: RetinoscopExam | null = null
        const hasRetinoscopData = Object.values(retinoscopFormData).some(value =>
          value !== undefined && value !== null && value !== ''
        );

        if (hasRetinoscopData) {
          if (retinoscopFormData.id) {
            updatedRetinoscopExam = await updateRetinoscopExam({
              ...retinoscopFormData,
              layout_instance_id: activeLayout.id
            });
          } else {
            updatedRetinoscopExam = await createRetinoscopExam({
              ...retinoscopFormData,
              layout_instance_id: activeLayout.id
            });
          }

          if (updatedRetinoscopExam) {
            setRetinoscopExam(updatedRetinoscopExam);
            setRetinoscopFormData({ ...updatedRetinoscopExam });
          }
        }
  ...
  const handleLayoutTabChange = async (tabId: number) => {
    ...
    try {
      ...
      if (!isNewMode) {
        ...
        const retinoscopData = await getRetinoscopExamByLayoutInstanceId(selectedTab.id)
        ...
        setRetinoscopFormData(retinoscopData || { layout_instance_id: selectedTab.id } as RetinoscopExam)
      }
  ...
  const handleEditButtonClick = () => {
    if (isEditing) {
      ...
      if (retinoscopExam) setRetinoscopFormData({ ...retinoscopExam });
      setIsEditing(true);
    }
  }
  ...
  const toolboxActions = createToolboxActions(
    ...
    finalSubjectiveFormData,
    additionFormData,
    (field: string, value: string) => handleOldRefractionFieldChange(field as keyof OldRefractionExam, value),
    (field: string, value: string) => handleObjectiveFieldChange(field as keyof ObjectiveExam, value),
    (field: string, value: string) => handleSubjectiveFieldChange(field as keyof SubjectiveExam, value),
    (field: string, value: string) => handleFinalSubjectiveChange(field as keyof FinalSubjectiveExam, value),
    (field: string, value: string) => handleAdditionFieldChange(field as keyof AdditionExam, value)
  )
