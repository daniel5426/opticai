...
def _find_external_document(documents_dir: Optional[str], account_code: str, file_code: str, file_name: str) -> Optional[Path]:
    ...
    for candidate in root.rglob("*"):
        ...
        if name_stem and name_stem in lowered_name:
            best = candidate
        elif account and account.lower() in lowered_parts:
            best = candidate
    return best
...
def _store_migrated_file(
    ...
    ext, detected_type = guess_file_extension_and_type(data, file_name)
    content_type = mime_type or detected_type or "application/octet-stream"
    safe_ext = ext or Path(file_name).suffix or ".bin"
    storage_key = f"clinics/{clinic_id}/clients/{client_id}/softoptic/{uuid.uuid4().hex}{safe_ext}"
    storage_service.upload(settings.SUPABASE_BUCKET, storage_key, data, content_type)
    return settings.SUPABASE_BUCKET, storage_key, len(data), content_type
...
def migrate_files(
    ...
    matched_documents: Optional[Set[str]] = None,
):
    t0 = time.time()
    ...
    for r in rows_iter:
        ...
        if not saved_path:
            ...
            if external_path:
                ...
                file_size = external_path.stat().st_size
                file_type = mimetypes.guess_type(external_path.name)[0] or "application/octet-stream"
                if matched_documents is not None:
                    try:
                        matched_documents.add(str(external_path.relative_to(Path(documents_dir))))
                    except Exception:
                        matched_documents.add(str(external_path))

        storage_bucket = "legacy-local"
        storage_key = saved_path or f"legacy/{r.get('code') or file_name}"
        if storage_service and saved_path:
            storage_bucket, storage_key, file_size, file_type = _store_migrated_file(
                storage_service,
                clinic_id,
                client_id,
                file_code,
                saved_name or file_name,
                saved_path,
                file_type,
            )
